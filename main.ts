game.splash("Jsi malý mimozemšťánek na neznáme zemi, který se potřebuje dostat na domovskou planetu")
game.splash("Pozor, na tvé cestě tě zkusí zastavit spousta překážek, vyhýbej se jim")
game.splash("Možná narazíš i na nějaké palivo, či munici. Ty se pokus sebrat")
let level = 0
let raketa = sprites.create(img`
    .......................
    ...........f...........
    ..........f1f..........
    .........f111f.........
    ........f11111f........
    ........f111111f.......
    .......f1111111f.......
    ......f111111111f......
    ......fdddddddddf......
    ......fddbddddbdf......
    .....fdddddddddddf.....
    .ffffdddddddddddddffff.
    fdddddddbddddddbddddddf
    fdddddddbbbbbbbbddddddf
    .fdddddddddbbddddddddf.
    ..ffffddddddddddddfff..
    ......fddddddddddf.....
    ......fddddddddddf.....
    ......fddddddddddf.....
    ......fddddddddddf.....
    ......fddddddddddf.....
    ......fddddddddddf.....
    ......ffffffffffff.....
    ......255555555552.....
    ......225555555522.....
    ........22222222.......
    `, SpriteKind.Player)
raketa.setStayInScreen(true)
raketa.setHitbox()
controller.moveSprite(raketa)
info.player1.setLife(3)
scene.setBackgroundImage(img`
    
    `)
/// Krásná hudba

forever(function() { 
   music.playMelody("Eb Bb Eb Eb F Gb Eb Eb Gb F Db Db F Gb Eb Eb Eb Bb Eb Eb F Gb Eb Eb Gb Bb Ab Gb F Db Gb Eb Eb", 200)
 /// music.playMelody("B AG#E F#F# c# BA G#G#G#B AG# F#F# ag# a g# a F#F# a g# a g# a F#F# c# BA G#G#G#B AG#", 200)

})
game.onUpdateInterval(500, function() {

    
        if(level == 0 ){
                let zizala = sprites.create(assets.image`Zizala`, SpriteKind.Enemy)
                zizala.setPosition(randint(0, 200), 0)
                zizala.setVelocity(0, randint(30, 100))
                zizala.setKind(SpriteKind.Enemy)

        }
        if(level == 1) {
    
                let trpaslik = sprites.create(assets.image`Trpaslik`, SpriteKind.Enemy)
                trpaslik.setPosition(randint(0, 200), 0)
                trpaslik.setVelocity(0, randint(10, 100))
                trpaslik.setKind(SpriteKind.Enemy)

                if (randint(0,1) == 1) {
                    let hafik = sprites.create(assets.image`Doggo OwO`, SpriteKind.Player)
                    hafik.setPosition(0, randint(50, 120))
                    hafik.setVelocity(randint(10, 100), 0)
                    hafik.setKind(SpriteKind.Enemy)
                }

        }
        if(level == 2){
            if (randint(0 , 3) == 3){
                let Palivo = sprites.create(assets.image`Fuel`, SpriteKind.Food)
                Palivo.setPosition(randint(0, 200), 0)
                Palivo.setVelocity(0, randint(10, 100))
                Palivo.setKind(SpriteKind.Food)
            }
            let krakra=sprites.create(assets.image`Krákoráček`, SpriteKind.Enemy)
            krakra.setPosition(0, randint(0, 120))
            krakra.setVelocity(randint(10, 100), 0)
            krakra.setKind(SpriteKind.Enemy)
            let Holub = sprites.createProjectileFromSide(assets.image`Ptacek`, randint(10, 50), randint(10, 50))
            Holub.setKind(SpriteKind.Enemy)
        }
    if (level == 3) {
        if (randint(0, 2) == 2) {
            let Palivo = sprites.create(assets.image`Fuel`, SpriteKind.Food)
            Palivo.setPosition(randint(0, 200), 0)
            Palivo.setVelocity(0, randint(10, 200))
            Palivo.setKind(SpriteKind.Food)
        }
        if (randint(0, 4) == 4) {
            let Munition = sprites.create(assets.image`Munition`, SpriteKind.Projectile)
            Munition.setPosition(randint(0, 200), 0)
            Munition.setVelocity(0, randint(10, 200))
            Munition.setKind(SpriteKind.Projectile)
        }
        let krakra = sprites.create(assets.image`Krákoráček`, SpriteKind.Enemy)
        krakra.setPosition(0, randint(80, 120))
        krakra.setVelocity(randint(10, 100), 0)
        krakra.setKind(SpriteKind.Enemy)
        let holub =sprites.create(assets.image`Ptacek`, SpriteKind.Enemy)
        holub.setPosition(0, randint(0, 120))
        holub.setVelocity(randint(10, 100), 0)
        holub.setKind(SpriteKind.Enemy)
        if (randint(0, 5) == 5){
            let letadlo = sprites.createProjectileFromSide(assets.image`Letadýlko`, randint(10, 100), randint(10, 100))
            letadlo.setKind(SpriteKind.Enemy)
        }
    }
    if (level == 4) {
        if (randint(0, 2) == 2) {
            let Palivo = sprites.create(assets.image`Fuel`, SpriteKind.Food)
            Palivo.setPosition(randint(0, 200), 0)
            Palivo.setVelocity(0, randint(10, 200))
            Palivo.setKind(SpriteKind.Food)
        }
        if (randint(0, 4) == 4) {
            let Munition = sprites.create(assets.image`Munition`, SpriteKind.Projectile)
            Munition.setPosition(randint(0, 200), 0)
            Munition.setVelocity(0, randint(10, 200))
            Munition.setKind(SpriteKind.Projectile)
        }
        let MeteorSusenka = sprites.create(assets.image`Meteor1`, SpriteKind.Enemy)
        MeteorSusenka.setPosition(randint(0, 200), 0)
        MeteorSusenka.setVelocity(0, randint(10, 100))
        MeteorSusenka.setKind(SpriteKind.Enemy)
        let MeteorFazole = sprites.create(assets.image`Meteor2`, SpriteKind.Enemy)
        MeteorFazole.setPosition(randint(0, 200), 0)
        MeteorFazole.setVelocity(0, randint(10, 100))
        MeteorFazole.setKind(SpriteKind.Enemy)
    }
    if (level == 5) {
        if (randint(0, 2) == 2) {
            let Palivo = sprites.create(assets.image`Fuel`, SpriteKind.Food)
            Palivo.setPosition(randint(0, 200), 0)
            Palivo.setVelocity(0, randint(10, 200))
            Palivo.setKind(SpriteKind.Food)
        }
        if (randint(0, 4) == 4) {
            let Munition = sprites.create(assets.image`Munition`, SpriteKind.Projectile)
            Munition.setPosition(randint(0, 200), 0)
            Munition.setVelocity(0, randint(10, 200))
            Munition.setKind(SpriteKind.Projectile)
        }
        if (randint(0,4) == 4){
            let letadlo = sprites.createProjectileFromSide(assets.image`Alien2`, randint(10, 100), randint(10, 100))

        }
        if (randint(0,4) == 3) {
            let letadlo = sprites.createProjectileFromSide(assets.image`Alien1`, randint(10, 100), randint(10, 100))
        }
        if (randint(0,2) == 2){
            let zlutasek = sprites.create(assets.image`Alien2`, SpriteKind.Enemy)
            zlutasek.setPosition(randint(0, 200), 0)
            zlutasek.setVelocity(0, randint(10, 100))
            zlutasek.setKind(SpriteKind.Enemy)
        }
        if (randint(0,5) == 5) {
            let Zelenacek = sprites.create(assets.image`Alien1`, SpriteKind.Enemy)
            Zelenacek.setPosition(randint(0, 200), 0)
            Zelenacek.setVelocity(0, randint(10, 100))
            Zelenacek.setKind(SpriteKind.Enemy)
        }

    }
    if (level == 6) {
        pause(5000)
        game.splash("Vyhrál jsi, dostal jsi se na svou domovskou planetu")
        game.over(false)
    }
       
})
            


    
      
            
         
        

         
        

          
      

          
       
    
/// Změna obtížností na jisté úrovně


/// Změna počtu životů při chycení srdíčka, nebo poškození nepřítelem
forever(function() {
    
    sprites.onOverlap(SpriteKind.Player, SpriteKind.Enemy, function(){
        info.player1.changeLifeBy(-1) 
        sprites.destroyAllSpritesOfKind(SpriteKind.Enemy, effects.fire, 100)
    }) 
    sprites.onOverlap(SpriteKind.Player, SpriteKind.Food, function () {
        info.player1.changeLifeBy(+1)
        sprites.destroyAllSpritesOfKind(SpriteKind.Food, effects.hearts, 100)

    })
    sprites.onOverlap(SpriteKind.Player, SpriteKind.Projectile, function () {
        sprites.destroyAllSpritesOfKind(SpriteKind.Enemy)
        sprites.destroyAllSpritesOfKind(SpriteKind.Projectile, effects.halo, 100)

    })
        

})



/// Změna úrovní, raketka přeci letí nahoru :-)
game.onUpdateInterval(100, function () {
    info.changeScoreBy(1)

    if (info.score() == 100) {
        scene.setBackgroundImage(img`
            
            `)
        level = 1
    }
    if (info.score() == 200) {
        scene.setBackgroundImage(img`
            
            `)
        level = 2
    }

    if (info.score() == 300) {
        scene.setBackgroundImage(img`
            
            `)
        level = 3
    }
    if (info.score() == 400) {
        scene.setBackgroundImage(img`
            
            `)
        level = 4
    }
    if (info.score() == 500) {
        scene.setBackgroundImage(img`
            
            `)
        level = 5
    }
    if (info.score() == 600) {
        scene.setBackgroundImage(img`
            
            `)
        level = 6
    }
    
})
